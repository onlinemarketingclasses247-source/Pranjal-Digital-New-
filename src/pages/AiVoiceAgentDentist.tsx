...
// --- Video Section with Auto-play and Sound ---
  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-gradient-to-br from-[#0a0f1c] to-[#0d1220]">
        <div className="absolute top-3 left-3 z-10 flex items-center gap-2 bg-black/60 backdrop-blur-sm rounded-full px-2 py-1 sm:px-3 sm:py-1.5">
          ...
          <span className="text-white text-[10px] sm:text-xs font-medium tracking-wider">🔴 WATCH LIVE DEMO</span>
        </div>
        
        <button
          onClick={handleMuteToggle}
          className="absolute top-3 right-3 z-10 bg-black/60 backdrop-blur-sm rounded-full p-1.5 sm:p-2 hover:bg-black/80 transition-colors"
        >
          {isMuted ? <VolumeX size={14} className="sm:size-16 text-white" /> : <Volume1 size={14} className="sm:size-16 text-white" />}
        </button>
        ...
        <div className="relative aspect-video bg-black/50">
          ...
          <button
            ...
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full bg-[#c9a84c]/90 backdrop-blur-sm flex items-center justify-center hover:bg-[#c9a84c] hover:scale-110 transition-all duration-300 shadow-2xl z-20"
          >
            {isPlaying ? <Pause size={20} className="sm:size-24 md:size-28 text-[#080c14]" /> : <Play size={20} className="sm:size-24 md:size-28 text-[#080c14] ml-0.5 sm:ml-1" />}
          </button>
        </div>
      </div>
      ...
      <div className="text-center mt-4">
        <p className="text-white/70 text-xs sm:text-sm flex items-center justify-center gap-2">
          <Video size={12} className="sm:size-14 text-[#c9a84c]" /> Watch Live AI Voice Assistant Product Demo Now
        </p>
      </div>
...
const ProblemSection = () => {
  return (
    <div className="bg-gradient-to-br from-[#0a0f1c] to-[#080c14] rounded-2xl p-5 sm:p-6 md:p-8 border border-white/10">
      <div className="text-center mb-5 sm:mb-6">
        <span className="inline-flex items-center gap-2 bg-red-500/10 px-3 py-1 rounded-full mb-3">
          <AlertCircle size={12} className="text-red-400" />
          <span className="text-red-400 text-[10px] sm:text-xs font-medium uppercase tracking-wider">The Problem</span>
        </span>
        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-2">Every Missed Call = Lost Revenue</h2>
        <p className="text-red-400 text-xs sm:text-sm font-semibold">Clinics lose 30–40% of calls — that's thousands in lost revenue every month</p>
      </div>
      
      <div className="grid md:grid-cols-2 gap-6 sm:gap-8 items-center mt-4 sm:mt-6">
        <div>
          <ul className="space-y-2 sm:space-y-3">
            <li className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white/5 rounded-lg"><X size={14} className="sm:size-16 text-red-400 flex-shrink-0" /><span className="text-white/70 text-xs sm:text-sm">Patients call when you're busy with other patients</span></li>
            <li className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white/5 rounded-lg"><X size={14} className="sm:size-16 text-red-400 flex-shrink-0" /><span className="text-white/70 text-xs sm:text-sm">No one answers after clinic hours or on weekends</span></li>
            <li className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white/5 rounded-lg"><X size={14} className="sm:size-16 text-red-400 flex-shrink-0" /><span className="text-white/70 text-xs sm:text-sm">Receptionist burnout and high turnover</span></li>
            <li className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white/5 rounded-lg"><X size={14} className="sm:size-16 text-red-400 flex-shrink-0" /><span className="text-white/70 text-xs sm:text-sm">No call tracking means no follow-ups</span></li>
          </ul>
        </div>
        
        <div className="bg-gradient-to-br from-red-500/10 to-[#080c14] border border-red-500/20 rounded-xl p-4 sm:p-5">
          <div className="flex items-center gap-2 mb-3 sm:mb-4">
            <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-red-500 animate-pulse" />
            <span className="text-white/70 text-xs sm:text-sm font-medium">Missed Calls This Week</span>
          </div>
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="bg-white/5 rounded-lg p-2 sm:p-3 flex justify-between items-center">
                <div>
                  <span className="text-white/80 text-xs sm:text-sm font-medium">{call.patient}</span>
                  <p className="text-white/30 text-[8px] sm:text-[10px]">{call.time}</p>
                </div>
                <span className="text-red-400/80 text-xs sm:text-sm font-medium">-{call.value}</span>
              </div>
            ))}
          </div>
          <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-red-500/20 rounded-lg text-center">
            <p className="text-red-400 text-xs sm:text-sm font-bold">Total Lost: $1,350 this week</p>
            <p className="text-red-400/60 text-[8px] sm:text-[10px] mt-1">That's over $5,400 per month!</p>
          </div>
        </div>
      </div>
    </div>
        ...
        {steps.map((step, idx) => (
          ...
          >
            ...
            <motion.div
              ...
              className="bg-gradient-to-br from-[#0a0f1c] to-[#0d1220] border rounded-xl p-4 sm:p-5 relative overflow-hidden h-full"
            >
              ...
              <div className="flex justify-between items-start mb-2 sm:mb-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-[#c9a84c]/20 to-[#c9a84c]/5 flex items-center justify-center"><step.icon size={16} className="sm:size-20 text-[#c9a84c]" /></div>
                <span className="text-xl sm:text-2xl font-bold text-white/10">{step.step}</span>
              </div>
              <h3 className="text-base sm:text-lg font-bold text-white mb-1 sm:mb-2">{step.title}</h3>
              <p className="text-white/50 text-[10px] sm:text-xs mb-2 sm:mb-3">{step.description}</p>
              <div className="flex flex-wrap gap-1 mt-1 sm:mt-2 pt-1 sm:pt-2 border-t border-white/10">
                {step.features.map((feature, fIdx) => (<span key={fIdx} className="text-[#c9a84c]/60 text-[8px] sm:text-[10px] bg-[#c9a84c]/5 px-1.5 sm:px-2 py-0.5 rounded-full">{feature}</span>))}
              </div>
              {activeStep === idx && (<div className="absolute bottom-2 sm:bottom-3 left-1/2 -translate-x-1/2 flex gap-1"><div className="w-1 h-1 rounded-full bg-[#c9a84c] animate-pulse" /><div className="w-1 h-1 rounded-full bg-[#c9a84c]/50 animate-pulse" style={{ animationDelay: '0.2s' }} /><div className="w-1 h-1 rounded-full bg-[#c9a84c]/30 animate-pulse" style={{ animationDelay: '0.4s' }} /></div>)}
            </motion.div>
          </motion.div>
        ))}
      </div>
      <div className="flex justify-center gap-2 mt-4 sm:mt-6">
        {steps.map((_, idx) => (<button key={idx} onClick={() => setActiveStep(idx)} className={`h-1 rounded-full transition-all duration-300 ${activeStep === idx ? 'w-5 sm:w-6 bg-[#c9a84c]' : 'w-1.5 sm:w-2 bg-white/20'}`} />))}
      </div>
    </div>
  );
};
...
const CollaborationSection = () => {
  return (
    <div className="bg-gradient-to-br from-[#0a0f1c] to-[#0d1220] rounded-2xl p-5 sm:p-6 md:p-8 border border-white/10">
      <div className="grid md:grid-cols-2 gap-6 sm:gap-8 items-center">
        <div>
          <div className="inline-flex items-center gap-2 bg-[#c9a84c]/10 px-3 py-1 rounded-full mb-3 sm:mb-4">
            <HeartHandshake size={12} className="sm:size-14 text-[#c9a84c]" />
            <span className="text-[#c9a84c] text-[10px] sm:text-xs font-medium">Human + AI Collaboration</span>
          </div>
          <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-3 sm:mb-4">We're Not Trying to Replace Your Receptionist — We're Empowering Them</h3>
          <p className="text-white/60 text-xs sm:text-sm md:text-base mb-3 sm:mb-4">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3 mt-4 sm:mt-6">
            <div className="bg-white/5 rounded-lg sm:rounded-xl p-2 sm:p-3"><TrendingUp size={14} className="sm:size-16 text-[#c9a84c] mb-1 sm:mb-2" /><p className="text-white/70 sm:text-white/80 text-xs sm:text-sm font-medium">3x Higher Productivity</p><p className="text-white/30 text-[8px] sm:text-[10px]">AI handles 70% of routine calls</p></div>
            <div className="bg-white/5 rounded-lg sm:rounded-xl p-2 sm:p-3"><UserCheck size={14} className="sm:size-16 text-[#c9a84c] mb-1 sm:mb-2" /><p className="text-white/70 sm:text-white/80 text-xs sm:text-sm font-medium">Less Burnout</p><p className="text-white/30 text-[8px] sm:text-[10px]">Receptionists focus on patients</p></div>
          </div>
        </div>
        <div className="relative">
          <div className="absolute inset-0 bg-gradient-to-r from-[#c9a84c]/20 to-transparent blur-3xl" />
          <div className="relative bg-gradient-to-br from-[#c9a84c]/10 to-[#080c14] border border-[#c9a84c]/20 rounded-xl p-4 sm:p-5">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              <div className="flex items-center gap-2"><div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-[#c9a84c]/20 flex items-center justify-center"><Bot size={12} className="sm:size-14 text-[#c9a84c]" /></div><span className="text-white/70 sm:text-white/80 text-[10px] sm:text-xs font-medium">AI Voice Agent</span></div>
              <div className="flex items-center gap-2"><div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-green-500 animate-pulse" /><span className="text-white/40 text-[8px] sm:text-[10px]">Active 24/7</span></div>
            </div>
            <div className="space-y-1.5 sm:space-y-2">
              <div className="bg-white/5 rounded-lg p-1.5 sm:p-2"><p className="text-white/60 text-[9px] sm:text-[11px]">AI: "Dr. Smith is with a patient. Can I help book an appointment?"</p></div>
              <div className="bg-[#c9a84c]/10 rounded-lg p-1.5 sm:p-2"><p className="text-[#c9a84c] text-[9px] sm:text-[11px]">Patient: "Yes, I need a cleaning next week."</p></div>
              <div className="bg-white/5 rounded-lg p-1.5 sm:p-2"><p className="text-white/60 text-[9px] sm:text-[11px]">AI: "Tuesday at 2pm works. I'll book it and add you to the system."</p></div>
              <div className="border-t border-white/10 pt-1.5 sm:pt-2 mt-1.5 sm:mt-2 text-center"><p className="text-white/30 text-[8px] sm:text-[10px]">Seamless handoff to human during business hours</p></div>
            </div>
          </div>
        </div>
      </div>
...
const ComparisonTable = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-[#0a0f1c] to-[#0d1220] rounded-2xl p-4 sm:p-6 border border-white/10 overflow-x-auto">
      <h3 className="text-base sm:text-lg font-bold text-white text-center mb-4 sm:mb-6">Why AI Voice Agent is Better Than Traditional Voicemail or Answering Machines</h3>
      <table className="w-full min-w-[450px] sm:min-w-[500px]">
        <thead><tr className="border-b border-white/10"><th className="text-left text-white/60 text-[10px] sm:text-sm font-medium py-2 sm:py-3 px-1 sm:px-2">Feature</th><th className="text-center text-[#c9a84c] text-[10px] sm:text-sm font-medium py-2 sm:py-3 px-1 sm:px-2">AI Voice Agent</th><th className="text-center text-white/40 text-[10px] sm:text-sm font-medium py-2 sm:py-3 px-1 sm:px-2">Voicemail</th><th className="text-center text-white/40 text-[10px] sm:text-sm font-medium py-2 sm:py-3 px-1 sm:px-2">Answering Machine</th></tr></thead>
        <tbody>
          {comparisons.map((item, idx) => (
            <tr key={idx} className="border-b border-white/5">
              <td className="text-white/70 text-[10px] sm:text-sm py-2 sm:py-3 px-1 sm:px-2">{item.feature}</td>
              <td className="text-center py-2 sm:py-3 px-1 sm:px-2">{typeof item.ai === 'boolean' ? (item.ai ? <CheckCircle2 size={12} className="sm:size-16 text-[#c9a84c] mx-auto" /> : <X size={10} className="sm:size-14 text-red-400 mx-auto" />) : <span className="text-[#c9a84c] text-[10px] sm:text-sm">{item.ai}</span>}</td>
              <td className="text-center py-2 sm:py-3 px-1 sm:px-2">{typeof item.voicemail === 'boolean' ? (item.voicemail ? <CheckCircle2 size={12} className="sm:size-16 text-green-500/50 mx-auto" /> : <X size={10} className="sm:size-14 text-red-400 mx-auto" />) : <span className="text-white/40 text-[10px] sm:text-sm">{item.voicemail}</span>}</td>
              <td className="text-center py-2 sm:py-3 px-1 sm:px-2">{typeof item.answering === 'boolean' ? (item.answering ? <CheckCircle2 size={12} className="sm:size-16 text-green-500/50 mx-auto" /> : <X size={10} className="sm:size-14 text-red-400 mx-auto" />) : <span className="text-white/40 text-[10px] sm:text-sm">{item.answering}</span>}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div className="mt-4 sm:mt-6 p-2 sm:p-4 bg-[#c9a84c]/10 rounded-lg text-center"><p className="text-[#c9a84c] text-[10px] sm:text-sm font-medium">🎯 AI Voice Agent captures 3x more appointments than voicemail</p></div>
    </div>
  );
};
...
const ProductivityBoost = () => {
  return (
    <div className="grid md:grid-cols-2 gap-5 sm:gap-8 items-center">
      <div className="relative"><div className="absolute inset-0 bg-gradient-to-r from-[#c9a84c]/20 to-transparent blur-3xl" /><div className="relative bg-gradient-to-br from-red-500/10 to-[#080c14] border border-red-500/20 rounded-xl p-4 sm:p-6"><div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4"><BarChart3 size={18} className="sm:size-24 text-red-400" /><span className="text-white font-bold text-base sm:text-lg">Before AI Voice Agent</span></div><ul className="space-y-2 sm:space-y-3"><li className="flex items-center gap-2 text-white/60 text-[10px] sm:text-sm"><X size={12} className="sm:size-14 text-red-400" /> Receptionist spends 4+ hours/day on calls</li><li className="flex items-center gap-2 text-white/60 text-[10px] sm:text-sm"><X size={12} className="sm:size-14 text-red-400" /> 30-40% of after-hours calls go unanswered</li><li className="flex items-center gap-2 text-white/60 text-[10px] sm:text-sm"><X size={12} className="sm:size-14 text-red-400" /> Manual data entry into CRM</li><li className="flex items-center gap-2 text-white/60 text-[10px] sm:text-sm"><X size={12} className="sm:size-14 text-red-400" /> High receptionist burnout & turnover</li></ul></div></div>
      <div className="relative"><div className="absolute inset-0 bg-gradient-to-l from-[#c9a84c]/20 to-transparent blur-3xl" /><div className="relative bg-gradient-to-br from-[#c9a84c]/10 to-[#080c14] border border-[#c9a84c]/20 rounded-xl p-4 sm:p-6"><div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4"><TrendingUp size={18} className="sm:size-24 text-[#c9a84c]" /><span className="text-white font-bold text-base sm:text-lg">After AI Voice Agent</span></div><ul className="space-y-2 sm:space-y-3"><li className="flex items-center gap-2 text-white/70 text-[10px] sm:text-sm"><CheckCircle2 size={12} className="sm:size-14 text-[#c9a84c]" /> AI handles 70% of routine calls</li><li className="flex items-center gap-2 text-white/70 text-[10px] sm:text-sm"><CheckCircle2 size={12} className="sm:size-14 text-[#c9a84c]" /> 100% of after-hours calls answered</li><li className="flex items-center gap-2 text-white/70 text-[10px] sm:text-sm"><CheckCircle2 size={12} className="sm:size-14 text-[#c9a84c]" /> Automatic CRM updates</li><li className="flex items-center gap-2 text-white/70 text-[10px] sm:text-sm"><CheckCircle2 size={12} className="sm:size-14 text-[#c9a84c]" /> 3x happier receptionists, lower turnover</li></ul></div></div>
    </div>
  );
};
...
const SolutionSection = () => {
  return (
    <div className="grid md:grid-cols-2 gap-5 sm:gap-6 max-w-3xl mx-auto">
      <div className="bg-gradient-to-br from-red-500/5 to-[#0a0f1c] border border-red-500/20 rounded-xl p-4 sm:p-6"><div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4"><XCircle size={18} className="sm:size-22 text-red-400" /><h3 className="text-white font-bold text-base sm:text-lg">Before AI</h3></div><ul className="space-y-2 sm:space-y-3"><li className="flex items-center gap-2 text-white/60 text-[10px] sm:text-sm"><X size={12} className="sm:size-14 text-red-400" /> Missed calls after hours</li><li className="flex items-center gap-2 text-white/60 text-[10px] sm:text-sm"><X size={12} className="sm:size-14 text-red-400" /> Manual appointment booking</li><li className="flex items-center gap-2 text-white/60 text-[10px] sm:text-sm"><X size={12} className="sm:size-14 text-red-400" /> Lost revenue</li></ul></div>
      <div className="bg-gradient-to-br from-[#c9a84c]/10 to-[#0a0f1c] border border-[#c9a84c]/30 rounded-xl p-4 sm:p-6 relative overflow-hidden group"><div className="absolute inset-0 bg-gradient-to-r from-[#c9a84c]/0 via-[#c9a84c]/5 to-[#c9a84c]/0 opacity-0 group-hover:opacity-100 transition-opacity duration-700" /><div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4"><ThumbsUp size={18} className="sm:size-22 text-[#c9a84c]" /><h3 className="text-white font-bold text-base sm:text-lg">With AI</h3></div><ul className="space-y-2 sm:space-y-3"><li className="flex items-center gap-2 text-white/70 text-[10px] sm:text-sm"><CheckCircle2 size={12} className="sm:size-14 text-[#c9a84c]" /> AI answers instantly 24/7</li><li className="flex items-center gap-2 text-white/70 text-[10px] sm:text-sm"><CheckCircle2 size={12} className="sm:size-14 text-[#c9a84c]" /> Auto appointment booking</li><li className="flex items-center gap-2 text-white/70 text-[10px] sm:text-sm"><CheckCircle2 size={12} className="sm:size-14 text-[#c9a84c]" /> Capture every call</li></ul></div>
    </div>
  );
};
...
const PricingSection = ({ onFreeTrialClick }) => {
  return (
    <div><div className="grid md:grid-cols-2 gap-5 sm:gap-6 max-w-4xl mx-auto">
      <div className="relative bg-gradient-to-br from-[#0a0f1c] to-[#0d1220] border-2 border-[#c9a84c]/50 rounded-xl p-5 sm:p-6 shadow-xl shadow-[#c9a84c]/15"><div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#c9a84c] text-[#080c14] text-[10px] sm:text-[11px] font-bold px-2 sm:px-3 py-0.5 sm:py-1 rounded-full">OUR PLAN</div><div className="relative text-center"><div className="w-10 h-10 sm:w-12 sm:h-12 mx-auto rounded-lg bg-[#c9a84c]/20 flex items-center justify-center mb-3 sm:mb-4"><Crown size={18} className="sm:size-22 text-[#c9a84c]" /></div><h3 className="text-lg sm:text-xl font-bold text-white">DentalVoiceAI</h3><div className="mt-3 sm:mt-4 mb-1"><span className="text-3xl sm:text-4xl font-bold text-[#c9a84c]">$49.99</span><span className="text-white/40 text-sm sm:text-base">/month</span></div><p className="text-[#c9a84c] text-[10px] sm:text-xs mb-3 sm:mb-4">+ nominal per-call fee for high volumes</p><ul className="space-y-1.5 sm:space-y-2 text-left mt-4 sm:mt-5 mb-4 sm:mb-6"><li className="flex items-center gap-2 text-white/80 text-[10px] sm:text-sm"><CheckCircle2 size={10} className="sm:size-12 text-[#c9a84c]" /> No setup fees</li><li className="flex items-center gap-2 text-white/80 text-[10px] sm:text-sm"><CheckCircle2 size={10} className="sm:size-12 text-[#c9a84c]" /> 24/7 availability</li><li className="flex items-center gap-2 text-white/80 text-[10px] sm:text-sm"><CheckCircle2 size={10} className="sm:size-12 text-[#c9a84c]" /> Free CRM integration</li><li className="flex items-center gap-2 text-white/80 text-[10px] sm:text-sm"><CheckCircle2 size={10} className="sm:size-12 text-[#c9a84c]" /> Low latency response</li></ul><button onClick={onFreeTrialClick} className="w-full bg-gradient-to-r from-[#c9a84c] to-[#dbb85c] text-[#080c14] font-bold py-2 sm:py-3 rounded-lg hover:shadow-lg transition-all text-xs sm:text-sm">Start Free Trial</button></div></div>
      <div className="bg-gradient-to-br from-[#0a0f1c] to-[#0d1220] border border-white/10 rounded-xl p-5 sm:p-6"><div className="text-center"><div className="w-10 h-10 sm:w-12 sm:h-12 mx-auto rounded-lg bg-white/5 flex items-center justify-center mb-3 sm:mb-4"><AlertCircle size={18} className="sm:size-22 text-white/40" /></div><h3 className="text-lg sm:text-xl font-bold text-white/60">Other AI Voice Agents</h3><div className="mt-3 sm:mt-4 mb-1"><span className="text-2xl sm:text-3xl font-bold text-white/40">$200+</span><span className="text-white/30 text-sm sm:text-base">/month</span></div><ul className="space-y-1.5 sm:space-y-2 text-left mt-4 sm:mt-5 mb-4 sm:mb-6"><li className="flex items-center gap-2 text-white/40 text-[10px] sm:text-sm"><X size={10} className="sm:size-12 text-red-400" /> High setup fees ($500+)</li><li className="flex items-center gap-2 text-white/40 text-[10px] sm:text-sm"><X size={10} className="sm:size-12 text-red-400" /> Expensive per-call charges</li><li className="flex items-center gap-2 text-white/40 text-[10px] sm:text-sm"><X size={10} className="sm:size-12 text-red-400" /> Slow response time</li><li className="flex items-center gap-2 text-white/40 text-[10px] sm:text-sm"><X size={10} className="sm:size-12 text-red-400" /> Long-term contracts</li></ul></div></div>
    </div></div>
  );
};
...
const CalendlySection = () => {
  ...
  return (
    <div id="calendly-section" className="scroll-mt-20"><div className="bg-gradient-to-br from-[#0a0f1c] to-[#0d1220] rounded-2xl p-4 sm:p-6 border border-white/10"><div className="text-center mb-4 sm:mb-5"><h3 className="text-lg sm:text-xl font-bold text-white mb-1">Schedule Your Free Demo</h3><p className="text-white/50 text-xs sm:text-sm">Choose a time that works for you. 30-min personalized walkthrough.</p></div><div className="rounded-xl overflow-hidden border border-white/10 min-h-[450px] sm:min-h-[550px] relative">{!isLoaded && (<div className="absolute inset-0 flex items-center justify-center bg-[#0a0f1c]"><div className="w-6 h-6 sm:w-8 sm:h-8 border-2 border-[#c9a84c]/30 border-t-[#c9a84c] rounded-full animate-spin" /></div>)}<iframe src={CALENDLY_URL} width="100%" height="600" frameBorder="0" className={`bg-white transition-opacity duration-300 ${isLoaded ? 'opacity-100' : 'opacity-0'}`} title="Calendly Booking" onLoad={() => setIsLoaded(true)} /></div></div></div>
  );
};
    ...
    <div className="relative min-h-[80vh] sm:min-h-[85vh] flex items-center pt-12 sm:pt-16 pb-12 sm:pb-16 overflow-hidden" onMouseMove={handleMouseMove}>
      ...
      <div className="relative text-center max-w-4xl mx-auto z-10 px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex items-center justify-center gap-2 sm:gap-3">
          <span className="inline-flex items-center gap-2 bg-[#c9a84c]/10 backdrop-blur-sm px-3 sm:px-4 py-1 rounded-full border border-[#c9a84c]/20">
          ...
          <div className="relative">
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-[#c9a84c]/20 flex items-center justify-center animate-pulse">
              <Activity size={12} className="sm:size-16 text-[#c9a84c]" />
            </div>
            <div className="absolute -inset-1 rounded-full bg-[#c9a84c]/30 animate-ping opacity-75" style={{ animationDuration: '0.8s' }} />
          </div>
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-white mb-4 sm:mb-5 leading-tight">Never Miss a Patient Call —<TypingEffect /></motion.h1>
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-white/60 text-xs sm:text-sm md:text-base mb-4 sm:mb-5 max-w-2xl mx-auto">AI Voice Receptionist that answers calls, books appointments, and updates your CRM automatically.</motion.p>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="inline-flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 bg-[#0a0f1c]/80 backdrop-blur-sm border border-white/10 rounded-full px-3 sm:px-4 py-1 sm:py-1.5 mb-5 sm:mb-6"><DollarIcon size={12} className="sm:size-14 text-[#c9a84c]" /><span className="text-white font-semibold text-[10px] sm:text-xs">Starts at $49.99/month</span><span className="text-white/40 text-[8px] sm:text-[10px] mx-0.5 sm:mx-1">•</span><span className="text-white/70 text-[10px] sm:text-xs">7-day free trial</span><span className="text-white/40 text-[8px] sm:text-[10px] mx-0.5 sm:mx-1">•</span><span className="text-white/70 text-[10px] sm:text-xs">No setup fees</span></motion.div>
        <CTAButtons onDemoClick={onDemoClick} onTrialClick={onFreeTrialClick} />
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.5 }} className="flex flex-wrap justify-center gap-3 sm:gap-4 mt-5 sm:mt-6 text-[8px] sm:text-[10px] md:text-xs text-white/40"><span className="flex items-center gap-1"><CheckCircle2 size={8} className="sm:size-10 text-[#c9a84c]" /> Works with your number</span><span className="flex items-center gap-1"><CheckCircle2 size={8} className="sm:size-10 text-[#c9a84c]" /> Setup in 24 hours</span><span className="flex items-center gap-1"><CheckCircle2 size={8} className="sm:size-10 text-[#c9a84c]" /> No technical skills</span></motion.div>
      </div>
    </div>
  );
};
    ...
    <div className="bg-[#080c14] min-h-screen font-sans antialiased overflow-x-hidden">
          ...
          {faqs.map((faq, idx) => (
            <div key={idx} className="border border-white/10 rounded-lg overflow-hidden">
              <button onClick={() => setActiveFaq(activeFaq === idx ? null : idx)} className="w-full flex justify-between items-center p-3 sm:p-4 text-left">
                <span className="text-white font-medium text-xs sm:text-sm">{faq.q}</span>
                <ChevronDown size={12} className={`text-white/50 transition-transform duration-300 ${activeFaq === idx ? 'rotate-180' : ''}`} />
              </button>
              {activeFaq === idx && (
                <div className="p-3 sm:p-4 pt-0 text-white/50 text-[10px] sm:text-xs border-t border-white/10 leading-relaxed">{faq.a}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <SectionWrapper className="py-8 sm:py-12">
        <div className="rounded-xl bg-gradient-to-br from-[#c9a84c]/10 to-[#0d1220] border border-[#c9a84c]/20 p-5 sm:p-8 text-center">
          ...
          <p className="text-white/60 mb-4 sm:mb-6 text-xs sm:text-sm">Join hundreds of dental practices automating their front desk with our AI voice agent.</p>
          <CTAButtons onDemoClick={scrollToCalendly} onTrialClick={() => setShowFreeTrial(true)} variant="small" />
        </div>
      </SectionWrapper>

      {/* Footer */}
      <div className="border-t border-white/10 py-5 sm:py-6 text-center">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-gradient-to-br from-[#c9a84c] to-[#dbb85c] flex items-center justify-center">
              <Mic size={12} className="sm:size-14 text-[#080c14]" />
            </div>
            <span className="text-white font-bold text-xs sm:text-sm">Dental<span className="text-[#c9a84c]">Voice</span>AI</span>
          </div>
          <div className="text-white/40 text-[9px] sm:text-[11px]">© 2024 DentalVoiceAI. AI Voice Receptionist for Dental Practices</div>
        </div>
      </div>
        ...
        {showCalendly && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-[#080c14] rounded-2xl w-full max-w-4xl h-[85vh] overflow-hidden border border-[#c9a84c]/30 shadow-2xl">
              <button onClick={() => setShowCalendly(false)} className="absolute top-4 right-4 z-10 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white/80 hover:bg-white/20 transition-colors border border-white/10"><X size={16} className="sm:size-20" /></button>
