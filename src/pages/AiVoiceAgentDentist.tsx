    ...
    >
      ...
      <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-gradient-to-br from-[#0a0f1c] to-[#0d1220]">
        ...
        <div className="relative aspect-video bg-black/50">
          ...
          >
            {isPlaying ? <Pause size={32} className="text-[#080c14]" /> : <Play size={32} className="text-[#080c14] ml-1" />}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8 items-center">
      <div>
        ...
        <ul className="space-y-3"><li className="flex items-center gap-3 p-3 bg-white/5 rounded-lg"><X size={16} className="text-red-400" /><span className="text-white/70 text-sm">Patients call when you're busy</span></li><li className="flex items-center gap-3 p-3 bg-white/5 rounded-lg"><X size={16} className="text-red-400" /><span className="text-white/70 text-sm">No answers after clinic hours</span></li><li className="flex items-center gap-3 p-3 bg-white/5 rounded-lg"><X size={16} className="text-red-400" /><span className="text-white/70 text-sm">Receptionist burnout</span></li></ul>
      </div>
      <div className="bg-gradient-to-br from-red-500/10 to-[#080c14] border border-red-500/20 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4"><div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /><span className="text-white/70 text-sm font-medium">Missed Calls This Week</span></div>
        <div className="space-y-2">{[{ patient: "Sarah Johnson", value: "$450" }, { patient: "Michael Chen", value: "$320" }, { patient: "Dr. Emily Wilson", value: "$580" }].map((call, i) => (<div key={i} className="bg-white/5 rounded-lg p-3 flex justify-between items-center"><span className="text-white/80 text-sm">{call.patient}</span><span className="text-red-400/80 text-sm font-medium">-{call.value}</span></div>))}</div>
        <div className="mt-4 p-3 bg-red-500/20 rounded-lg text-center"><p className="text-red-400 text-sm font-bold">Total Lost: $1,350 this week</p></div>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="relative min-h-[85vh] flex items-center pt-16 pb-16 overflow-hidden" onMouseMove={handleMouseMove}>
      ...
      <div className="relative text-center max-w-4xl mx-auto z-10 px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}><span className="inline-flex items-center gap-2 bg-[#c9a84c]/10 backdrop-blur-sm px-4 py-1.5 rounded-full mb-5 border border-[#c9a84c]/20"><Sparkles size={12} className="text-[#c9a84c] animate-pulse" /><span className="text-[#c9a84c] text-[10px] font-medium uppercase tracking-wider">Smart AI Receptionist for Dental Practices</span></span></motion.div>
...
export default function DentalVoiceAgentLanding() {
  ...
  return (
    <div className="bg-[#080c14] min-h-screen font-sans antialiased overflow-x-hidden">
      ...
      <SectionWrapper className="pt-0 pb-8"><AIWorkAnimation /></SectionWrapper>
      <SectionWrapper className="pt-0 pb-8"><FeatureCards /></SectionWrapper>
      <SectionWrapper className="pt-0 pb-12"><VideoSection onDemoClick={scrollToCalendly} /></SectionWrapper>
      <SectionWrapper className="pt-0 pb-6"><div className="text-center"><CTAButtons onDemoClick={scrollToCalendly} onTrialClick={() => setShowFreeTrial(true)} variant="small" /></div></SectionWrapper>
      <SectionWrapper className="py-8"><CollaborationSection /></SectionWrapper>
      <SectionWrapper className="py-8"><ComparisonTable /></SectionWrapper>
      <SectionWrapper className="py-8"><ProductivityBoost /></SectionWrapper>
      <SectionWrapper id="how-it-works" className="py-8"><SectionHeader badge="Simple Process" title="How It Works" description="A 3-step system that turns missed calls into booked appointments" /><HowItWorksFlow /></SectionWrapper>
      <SectionWrapper className="pt-0 pb-6"><div className="text-center"><CTAButtons onDemoClick={scrollToCalendly} onTrialClick={() => setShowFreeTrial(true)} variant="small" /></div></SectionWrapper>
      <SectionWrapper className="bg-gradient-to-b from-[#0a0f1c] to-[#080c14] py-12"><ProblemSection /></SectionWrapper>
      <SectionWrapper className="py-12"><SectionHeader badge="The Solution" title="Your 24/7 AI Receptionist That Converts Calls" center={true} /><SolutionSection /></SectionWrapper>
      <SectionWrapper id="pricing" className="py-12"><SectionHeader badge="Simple Pricing" title="Simple, Affordable Pricing" /><PricingSection onFreeTrialClick={() => setShowFreeTrial(true)} /></SectionWrapper>
      <SectionWrapper className="pt-0 pb-6"><div className="text-center"><CTAButtons onDemoClick={scrollToCalendly} onTrialClick={() => setShowFreeTrial(true)} variant="small" /></div></SectionWrapper>
      <SectionWrapper className="py-8"><CalendlySection /></SectionWrapper>
      <SectionWrapper className="bg-gradient-to-b from-[#0a0f1c] to-[#080c14] py-12"><SectionHeader badge="FAQ" title="Frequently Asked Questions" /><div className="max-w-2xl mx-auto space-y-3">{faqs.map((faq, idx) => (<div key={idx} className="border border-white/10 rounded-lg overflow-hidden"><button onClick={() => setActiveFaq(activeFaq === idx ? null : idx)} className="w-full flex justify-between items-center p-4 text-left"><span className="text-white font-medium text-sm">{faq.q}</span><ChevronDown size={14} className={`text-white/50 transition-transform duration-300 ${activeFaq === idx ? 'rotate-180' : ''}`} /></button>{activeFaq === idx && (<div className="p-4 pt-0 text-white/50 text-xs border-t border-white/10">{faq.a}</div>)}</div>))}</div></SectionWrapper>
      <SectionWrapper className="py-12"><div className="rounded-xl bg-gradient-to-br from-[#c9a84c]/10 to-[#0d1220] border border-[#c9a84c]/20 p-8 text-center"><h3 className="text-xl md:text-2xl font-bold text-white mb-3">Ready to Never Miss a Patient Call Again?</h3><p className="text-white/60 mb-6 text-sm">Join hundreds of dental practices automating their front desk with our AI voice agent.</p><CTAButtons onDemoClick={scrollToCalendly} onTrialClick={() => setShowFreeTrial(true)} variant="small" /></div></SectionWrapper>
      <div className="border-t border-white/10 py-6 text-center"><div className="flex flex-col sm:flex-row items-center justify-center gap-3"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#c9a84c] to-[#dbb85c] flex items-center justify-center"><Mic size={14} className="text-[#080c14]" /></div><span className="text-white font-bold text-sm">Dental<span className="text-[#c9a84c]">Voice</span>AI</span></div><div className="text-white/40 text-[11px]">© 2024 DentalVoiceAI. AI Voice Receptionist for Dental Practices</div></div></div>
      <AnimatePresence>{showCalendly && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md"><motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-[#080c14] rounded-2xl w-full max-w-4xl h-[85vh] overflow-hidden border border-[#c9a84c]/30 shadow-2xl"><button onClick={() => setShowCalendly(false)} className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white/80 hover:bg-white/20 transition-colors border border-white/10"><X size={20} /></button><iframe src={CALENDLY_URL} width="100%" height="100%" frameBorder="0" className="bg-white" title="Calendly Booking" /></motion.div></div>)}</AnimatePresence>
